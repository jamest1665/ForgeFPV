...
func _ready():
	if database_path:
		...
		database = get_tree().get_first_node_in_group("drone_database") as DroneDatabase

	if main_menu_path:
		main_menu = get_node(main_menu_path) as MainMenu
...
func _on_drone_selected(index: int):
	...
	if drone_info:
		drone_info.text = "[b]%s[/b]\nType: %s\nSpeed: %.1f m/s" % [drone.display_name, drone.type, drone.max_speed]
